...
def run():
    ...
    def class_to_source_link(_cls):
        ...
        link = 'https://github.com/pyhashzero/pyhashzero/blob/master/' + _path + '#L' + str(line)
        return '[[source]](' + link + ')'
    ...
    for subdir, dirs, fnames in os.walk('templates'):
        for fname in fnames:
            new_subdir = subdir.replace('templates', 'docs')
            if not os.path.exists(new_subdir):
                os.makedirs(new_subdir)
            if fname[-3:] == '.md':
                fpath = os.path.join(subdir, fname)
                new_fpath = fpath.replace('templates', 'docs')
    ...
    for page_data in pages:
        ...
        path = os.path.join('docs/sources', page_name)
