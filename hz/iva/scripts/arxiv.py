...
from hz.db.mongo import MongoDatabase
from hz.db.record.constraint import UniqueConstraint
from hz.db.record.document import Document
...
class Task(BaseTask):
    def __init__(self, api=None):
        ...
        self.database = MongoDatabase(json.loads(open('key.json', 'r').read())['mongo']['connection'])
    ...
    def run(self):
        ...
        if self.action[0] == 'list':
            self.api.output(self.database.read_many(Tag), voice=False)
